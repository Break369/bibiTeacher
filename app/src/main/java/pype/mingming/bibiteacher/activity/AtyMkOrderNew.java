...
public class AtyMkOrderNew extends Activity implements AdapterView.OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        Intent i2 = new Intent(AtyMkOrderNew.this,AtyChat.class);
        i2.putExtra("userID",user);
        if (adapter.getItem(position).getEmployer().getUsername().equals(user.getUsername())){
            i2.putExtra("friendID",adapter.getItem(position).getEmployee());
        }else {
            i2.putExtra("friendID",adapter.getItem(position).getEmployer());
        }
        if (adapter.getItem(position).getChatDB()!=null){
            startActivity(i2);
        }else {
            startActivity(i);
        }
